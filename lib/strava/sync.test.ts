...
describe('syncActivities', () => {
  ...
  it('links an unlinked session that was manually marked complete', async () => {
    const supabase = createFakeSupabase({
      strava_tokens: [
        { id: 't1', user_id: 'u1', athlete_id: 42, access_token: 'a1', refresh_token: 'r1', expires_at: futureIso, last_synced_at: null },
      ],
      planned_sessions: [
        { id: 's1', user_id: 'u1', plan_id: 'p1', date: '2026-09-15', activity_type: 'running', session_name: 'Easy run', priority: 'essential', target_duration_min: 45, target_distance_km: null, target_dplus_m: null, intensity: null, instructions: null, status: 'done', linked_activity_id: null, created_at: '2026-09-01T00:00:00Z' },
      ],
    })
    vi.mocked(stravaClient.fetchActivitiesSince).mockResolvedValue([
      { id: 999, name: 'Morning run', type: 'Run', sport_type: 'Run', start_date: '2026-09-15T06:00:00Z', start_date_local: '2026-09-15T07:00:00Z', moving_time: 2700, distance: 8000, total_elevation_gain: 150, average_speed: 2.96 },
    ...
    const result = await syncActivities(supabase, 'u1')
    expect(result).toEqual({ imported: 1, matched: 1 })
    expect(supabase._tables.planned_sessions[0].linked_activity_id).not.toBeNull()
  })

  it('does not link an activity to a skipped session', async () => {
    const supabase = createFakeSupabase({
      strava_tokens: [
        { id: 't1', user_id: 'u1', athlete_id: 42, access_token: 'a1', refresh_token: 'r1', expires_at: futureIso, last_synced_at: null },
      ],
      planned_sessions: [
        { id: 's1', user_id: 'u1', plan_id: 'p1', date: '2026-09-15', activity_type: 'running', session_name: 'Easy run', priority: 'essential', target_duration_min: 45, target_distance_km: null, target_dplus_m: null, intensity: null, instructions: null, status: 'skipped', linked_activity_id: null, created_at: '2026-09-01T00:00:00Z' },
      ],
    })
    vi.mocked(stravaClient.fetchActivitiesSince).mockResolvedValue([
      { id: 999, name: 'Morning run', type: 'Run', sport_type: 'Run', start_date: '2026-09-15T06:00:00Z', start_date_local: '2026-09-15T07:00:00Z', moving_time: 2700, distance: 8000, total_elevation_gain: 150, average_speed: 2.96 },
